...
def get_ellipticity_data():
	...
	print(len(sorted(os.listdir(catalog_path))[0:-3]))
	ellipticity_data = []
	total_counts = []
	...
	for filename in files:
		with fits.open(catalog_path+filename) as member_file:		#member file .fit; cluster file .cat
			...
			for member in temp_member_data:
				if member[4]!=0 and any(float(mem_prob)>=prob for mem_prob in member[6]):
			...
			for ID in IDs:
				...
				#####
				ra = member_data[member_data.clusterID == ID].RA.values
				dec = member_data[member_data.clusterID == ID].Dec.values
				p = member_data[member_data.clusterID == ID]['prob'].values
				centre_ra = [(np.where(ra>330, ra-360.0, ra)*p).sum()]/p.sum()
				centre_ra[centre_ra<0]+=360.0
				centre_dec = (dec*p).sum()/p.sum()
				centre = pd.DataFrame(np.column_stack((centre_ra, centre_dec)), columns=['Xphys', 'Yphys'])
				#####
	...
	ellipticity_data.to_pickle("./ellipticity_df_modified_centre.pkl")

	return None

def plot_frequencies(ell_data):
	plt.figure(figsize = (6, 7))
	ell_data["angle"].hist(bins = 50, grid = False, color='#74c476', edgecolor = 'black')
	plt.xlabel(r"$\phi$")
	#plt.ylabel("Frequency")
	#plt.text(55, 65, "Mean = " + str(round(ell_data["angle"].mean(), 3)))
	plt.savefig('frequency_angle', bbox_inches = 'tight', dpi = 200)
	plt.close()

	plt.figure(figsize = (6, 7))
	np.absolute(ell_data["epsilon"]).hist(bins = 30, grid = False, color='#74c476', edgecolor = 'black')
	...
	print('E mean is', np.absolute(ell_data["epsilon"]).mean())
	np.absolute(ell_data["epsilon"]).mean()
	plt.savefig('frequency_absolute_epsilon_ppt', bbox_inches = 'tight', dpi = 300)
	plt.close()
...
def plot_epsilon_lambda(merged_df):
	...
	split_df = np.array_split(merged_df[merged_df.LAMBSTAR>=15].sort_values(by = 'LAMBSTAR'), 3)
	#err_color = ['mediumvioletred', 'purple', 'darkblue']																		#error bar colours
	#p_color = ['pink', 'plum', 'cornflowerblue']	
	#colors = ['#bae4b3', '#74c476', '#31a354']																			#point colours
	colors = ['#74c476', '#238b45', '#00441b']
	for i in range(3):
		...
		print(x, y, y_err)
		plt.scatter(split_df[i]["LAMBSTAR"], split_df[i]["abs_epsilon"], c=colors[i], alpha = 0.25, s = 1)
		plt.errorbar(x, y, yerr = y_err, fmt='.k', capsize = 3, color = colors[i], ecolor = colors[i], elinewidth = 1, capthick = 1)			#plotting the means and errorbars in the ith bin
		print(split_df[i]["LAMBSTAR"].min())
	plt.xlim(15, 100)
	plt.ylim(0, 0.5)
...
#ellipticity_df = pd.read_pickle("ellipticity_df_modified_centre.pkl")
#print(ellipticity_df.shape)
#plot_frequencies(ellipticity_df)
...
#merged_df.to_pickle("./merged_df_modified_centre.pkl")
